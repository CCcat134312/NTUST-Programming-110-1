...
string StringReplace(const string origin, const string replacement, const string template)
{
    string result = Initialize(NULL);
    for (int i = 0; i < origin.size; i++)
    {
        ...
        {
            StringCat(&result, .str = template.content);
...
void StringSliceExample();
void StringCatExample();
...
void StringSplitExample()
{
    puts("input a line:");
    string s = ReadLine();
    puts("input a separate char:");
    char separate = getchar();
    putchar('\n');
    int n;
    string *arr = StringSplit(s, separate, &n);
    printf("Total :%d\n", n);
    for (int i = 0; i < n; i++)
    {
        printf("String %d: ", i + 1);
        PrintStringInfo(arr[i]);
    }
}
void StringSliceExample()
{
    string s = Initialize("0123456789");
    PrintStringInfo(StringSlice(s, 2, 6));
    PrintStringInfo(StringSlice(s, .start = 2));
    PrintStringInfo(StringSlice(s, .end = 6));
}
void StringCatExample()
{
    string s1 = Initialize("123");
    string s2 = Initialize("456");

    StringCat(&s1, .charArr = "cat");
    PrintStringInfo(s1);
    StringCat(&s1, .str = s2);
    PrintStringInfo(s1);
    StringCat(&s1, "ccc", s2);
...
void StringReplaceExample()
{
    string s1 = Initialize("Hello_world!_world!_world!");
    string s2 = Initialize("world");
    string s3 = Initialize("cccccat");
    PrintStringInfo(StringReplace(s1, s2, s3));
}
void BUG()
{
    // BUG 1
    string s1 = Initialize("www");
    StringFree(&s1);
    PrintStringInfo(s1);
}